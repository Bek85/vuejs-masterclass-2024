...
import { taskQuery, type Task } from '@/utils/tasksQueries';
const route = useRoute('/tasks/[id]');

const task = ref<Task | null>(null);

watch(() => task.value?.name, () => {
  usePageStore().pageTitle = `Task: ${task.value?.name || ''}`;
})
...
<template>
  <Table v-if="task">
      ...
      <TableCell>
        <div class="flex">
          <Avatar class="-mr-4 border border-primary hover:scale-110 transition-transform"
            v-for="collaborator in task.collaborators" :key="collaborator">
    ...
    <TableRow class="hover:bg-transparent">
      ...
      <TableCell> Comments cards goes in here.. <div class="flex flex-col justify-between p-3 bg-muted my-2 rounded-md">
